...
func (j *Jsonc) Strip(from string) (to string) {
	...
	for j.index < j.len {
		...
		// 修剪数组或对象末尾的尾部逗号
		if j.comment == 0 && !j.inStr && ((j.inArray && char == "]") || (j.inObject && char == "}")) {
		...
		// 果内部字符串或外部注释，则按原样附加
		if j.inString(prev, char, next, original) || j.outsideComment(char, next) {
...
func (j *Jsonc) compliment(prev string, char string, next string) (compliment string) {
	if j.inStr && char == constant.Esc && constant.Enter == next {
		j.index++
		compliment = ""
	} else if cached, ok := j.spacesPair[char]; ok && j.inStr {
		compliment = cached
	} else if j.isNonStringValue(char, "+") && j.isNumber(next, false) {
		compliment = ""
	} else if j.isNonStringValue(char, ".") {
		compliment = j.decimalPointNumber(prev, next)
	} else if constant.SingleQuote == j.stringLabel {
		compliment = j.singleQuotedString(prev, char, next)
	}

	return
}

func (j *Jsonc) decimalPointNumber(prev string, next string) (point string) {
	prevNum, nextNum := j.isNumber(prev, false), j.isNumber(next, false)
	if !prevNum && nextNum {
		point = "0."
...
func (j *Jsonc) singleQuotedString(prev string, char string, next string) (single string) {
	if char+next == constant.Esc+constant.SingleQuote {
		char = constant.SingleQuote
		j.index++
	} else if prev != constant.Esc && char == constant.SingleQuote {
		char = constant.DoubleQuote
	} else if char == constant.DoubleQuote {
		char = `\"`
	}
	single = char
...
func (j *Jsonc) quoteKey(char string, wasQuoted bool) (quote string, quoted bool) {
	...
	if !j.inStr && inKey && !strings.ContainsAny(char, "[]{}'\",/*:\r\n\t ") {
		...
		j.inStr, quoted, j.stringLabel = true, true, constant.DoubleQuote
	}
	if j.inStr && wasQuoted && inKey && (char == ":" || char == " " || char == constant.SingleQuote) {
		quote = constant.DoubleQuote
		j.inStr, quoted, j.stringLabel = false, false, ""
	}

	return
}

func (j *Jsonc) inString(prev string, char string, next string, old string) bool {
	charNext := char + next
	maybeStr := (char == constant.DoubleQuote || char == constant.SingleQuote) && (!j.inStr || j.stringLabel == char)
	...
	if j.inStr && (charNext == `":` || charNext == `",` || charNext == `"]` || charNext == `"}`) {
		j.inStr = old+prev != constant.Esc+constant.Esc
	}
...
func (j *Jsonc) reset() {
	...
	j.inStr, j.inArray, j.inObject = false, false, false
